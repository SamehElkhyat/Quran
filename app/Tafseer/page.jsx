...
export default function TafsirPage() {
  ...
  const getData = async () => {
    try {
      ...
      const response = await axios.get(`https://www.mp3quran.net/api/v3/tafsir?tafsir=5&language=ar`);
      setData(response.data.tafasir.soar);
      setFilteredData(response.data.tafasir.soar);
    } catch (err) {
      console.error("Error fetching tafsir data:", err);
      setError("Failed to fetch Quran tafsir. Please try again.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <Container maxWidth="xl">
        ...
        >
          <Box sx={{ textAlign: 'center', mb: 6 }}>
            ...
            >
              Explore detailed interpretations and explanations of Quranic verses from renowned scholars 
              and Islamic scholars throughout history.
            </Typography>
          </Box>
        </motion.div>
            ...
            <CardContent sx={{ p: 4 }}>
              ...
              {query && (
                <Typography variant="body2" sx={{ mt: 2, color: 'rgba(255,255,255,0.7)' }}>
                  Found {filteredData.length} surah{filteredData.length !== 1 ? 's' : ''}
                </Typography>
              )}
            </CardContent>
          </Card>
          ...
          >
            <Box sx={{ textAlign: 'center', mt: 6 }}>
              ...
              <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.7)' }}>
                Try adjusting your search terms or browse all available tafsir.
              </Typography>
            </Box>
          </motion.div>
        )}
        ...
        >
          <Box sx={{ textAlign: 'center', mt: 6 }}>
            ...
            >
              All tafsir content is sourced from mp3quran.net and provides detailed interpretations 
              of Quranic verses from respected Islamic scholars and commentators.
            </Typography>
          </Box>
        </motion.div>
      </Container>
