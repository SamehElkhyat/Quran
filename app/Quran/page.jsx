...
export default function QuranPage() {
  ...
  const getData = async () => {
    try {
      ...
      setFilteredData(response.data.reciters);
    } catch (err) {
      console.error("Error fetching Quran data:", err);
      setError("Failed to fetch Quran recitations. Please try again.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <Container maxWidth="xl">
        ...
        >
          <Box sx={{ textAlign: 'center', mb: 6 }}>
            ...
            >
              Listen to beautiful Quran recitations from renowned reciters around the world
            </Typography>
          </Box>
        </motion.div>
            ...
            <CardContent sx={{ p: 4 }}>
              ...
              {query && (
                <Typography variant="body2" sx={{ mt: 2, color: 'rgba(255,255,255,0.7)' }}>
                  Found {filteredData.length} reciter{filteredData.length !== 1 ? 's' : ''}
                </Typography>
              )}
            </CardContent>
          </Card>
                    ...
                    <CardContent sx={{ p: 3, pt: 0 }}>
                      {item.moshaf && item.moshaf.length > 0 && (
                        <Box sx={{ mb: 2 }}>
                          ...
                          {item.moshaf.slice(0, 3).map((moshaf, mIndex) => (
                            <Chip 
                              key={mIndex}
                              label={String(moshaf.name).split("-").slice(0, 2).join(" ")}
                              size="small"
                              sx={{ 
                                mr: 1, 
                                mb: 1,
          ...
          >
            <Box sx={{ textAlign: 'center', mt: 6 }}>
              ...
              <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.7)' }}>
                Try adjusting your search terms or browse all available reciters.
              </Typography>
            </Box>
          </motion.div>
        )}
        ...
        >
          <Box sx={{ textAlign: 'center', mt: 6 }}>
            ...
            >
              All recitations are sourced from mp3quran.net and are available for educational and spiritual purposes.
            </Typography>
          </Box>
        </motion.div>
      </Container>
